...
function UserCard({
  ...
  let newLetter = false;
  let userCardStyle;
  userCardStyle = (page !== 'ShowFriends') ? [styles.container, { borderTopWidth: 1, borderColor: 'rgba(0, 0, 0, 0.25)' }] : styles.container

  if (
    (!prevFriend || name[0].toUpperCase() !== prevFriend.name[0].toUpperCase()) && page
  ) {
  ...
  } else if(page === 'FriendRequests') {
    userCardStyle = styles.container
  }
  ...
  let avatar = false;
  if (friend && Object.keys(friend).includes('avatar')) {
  ...
  return (
    <View>
      {newLetter}
      <TouchableOpacity style={userCardStyle} onPress={handler2}>
        {avatar}
